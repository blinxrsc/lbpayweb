...
// =====================================================================
// CHANGES FROM v1.2 (see chat for full rationale):
//  1. Non-blocking pulse output (state machine in loop()) instead of a
//     blocking for()+delay() loop, so client.loop() / MQTT keepalive /
//     the watchdog keep running while pulses are being sent.
//  2. Every REMOTE_START now carries an "op_id" (set by Laravel) and a
//     "pulses" count (already calculated server-side). The firmware no
//     longer re-derives pulses from price/0.10 — that logic now lives
//     in one place (Laravel), where discounts/promotions are also
//     applied.
//  3. Idempotency: a duplicate op_id (re-delivered command) is ignored
//     instead of starting the machine twice.
//  4. Busy protection: a new REMOTE_START arriving while pulses are
//     still running is rejected, not stacked on top.
//  5. Application-level ACK protocol published on the status topic:
//     "received" -> "completed" (or "busy"/"rejected"/"duplicate_ignored").
//     PubSubClient only supports QoS 0, so this app-level ack + a
//     backend-side timeout/retry is what actually makes the payment
//     flow reliable — not the MQTT transport alone.
//  6. Pulse width/delay/polarity and coin-input polarity are now
//     configurable via a "CONFIG" command and persisted in flash
//     (Preferences), matching the per-device config screen you showed
//     (Pulse Width, Pulse Delay, Pull Up/Down, Coin Signal Idle Level,
//     Coin Signal Sensitivity). Defaults match that screenshot.
// =====================================================================
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(action, "CONFIG") == 0) {
    // Backend pushes machine-specific pulse/coin settings.
    // Publish this with retain=true on a dedicated .../config topic so a
    // device that reboots or reconnects later immediately re-applies it
    // (safe to retain, unlike a payment/start command).
...
void setup() {
  ...
  String bootPayload = "{\"event\":\"boot\",\"status\":\"online\",\"version\":\"1.3.0\"}";
