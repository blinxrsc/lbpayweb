    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <form method="GET" action="{{ route('reports.outlet-revenue') }}" class="flex flex-wrap items-end gap-3">
                    <div>
                    ...
                    <a href="{{ route('reports.outlet-revenue') }}" class="inline-flex items-center px-4 py-2 text-sm text-gray-500 hover:text-gray-700">
                        Reset to This Month
                    </a>
                </form>
                <p class="text-xs text-gray-400 mt-3">
                    ...
                    @else
                        · All outlets
                    @endif
                    — device count is a current snapshot, not filtered by date range.
                </p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div class="bg-white p-5 rounded-lg border border-gray-200">
                    <div class="flex items-center gap-2 text-gray-500">
                        ...
                        <h4 class="text-xs font-semibold uppercase tracking-wide">Devices Installed</h4>
                    </div>
                    <p class="text-3xl font-semibold text-gray-900 mt-2">{{ $summary['total_devices'] }}</p>
                </div>
                <div class="bg-white p-5 rounded-lg border border-gray-200">
                    <div class="flex items-center gap-2 text-gray-500">
                        ...
                        <tbody>
                            @forelse($byOutlet as $row)
                                <tr class="text-sm border-b border-gray-100 last:border-0 hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-2 text-right">{{ $row['device_count'] }}</td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-4 py-6 text-center text-gray-400">No revenue recorded for this period.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
